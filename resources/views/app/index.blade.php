...
@push('head')
    <style>
        ...
        @media print {
            ...
            @page {
                margin: 10mm;
                /* Matches the screen preview sheet padding exactly */
            }

            .a4-page-container {
                padding: 0 !important;
                /* Handled by page margin */
                margin: 0 !important;
...
@section('content')
    <div class="pt-app-scope min-h-screen flex flex-col font-sans bg-[#faf9f6]" x-data="appWorkspace()" x-init="initApp()">
        ...
        <header
            class="no-print h-[--toolbar-height] shrink-0 bg-white border-b border-[--border] flex items-center justify-between px-6 z-10 shadow-sm">
            <div class="flex items-center gap-4">
                <a href="{{ route('home') }}" class="group flex items-center">
                    <span
                        class="font-mono text-xs uppercase tracking-[0.25em] font-semibold text-[#141428] group-hover:text-[#3b3c95] transition-colors duration-200">
                        PAPER<span
                            class="text-[#3b3c95] font-black group-hover:text-[#141428] transition-colors duration-200">/</span>TRAIL
                    </span>
                </a>
                <div class="w-px h-4 bg-slate-200" x-show="$store.spreadsheet.isLoaded"></div>
                <span class="text-xs text-[--ink-muted] font-medium hidden sm:inline" x-show="$store.spreadsheet.isLoaded"
            ...
            <div class="flex items-center gap-3" x-cloak x-show="$store.spreadsheet.isLoaded">
                ...
                <!-- Table Styles -->
                <select @change="$store.theme.setTableStyle($event.target.value)" :value="$store.theme.tableStyle"
                    class="text-xs border border-[--border] rounded-lg px-3 py-1.5 bg-white text-[--ink] outline-none">
                ...
                <button @click="triggerPrint()"
                    class="bg-[#3b3c95] text-white px-4 py-1.5 rounded-lg text-xs font-semibold hover:opacity-90 active:scale-[0.98] transition-all shadow-sm">
                    🖨️ Print / Save PDF
                </button>
            </div>
        </header>
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            <aside class="no-print workspace-sidebar shrink-0 overflow-y-auto p-5 space-y-6" x-cloak
                x-show="$store.spreadsheet.isLoaded">

                <!-- Back to Import -->
                <button @click="clearActiveFile()"
                    class="text-xs font-semibold text-[--ink-muted] hover:text-[--ink] flex items-center gap-1.5 transition-colors pb-2 border-b border-slate-100 w-full text-left">
                ...
                <div class="space-y-2" x-show="$store.spreadsheet.sheetNames.length > 1">
                    ...
                    <select @change="$store.spreadsheet.selectSheet($event.target.value)"
                        :value="$store.spreadsheet.activeSheet"
                        class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 bg-white text-[--ink] outline-none">
                ...
                <div class="space-y-3">
                    ...
                    <div class="space-y-2.5">
                        <input type="text" x-model="$store.letterhead.active.companyName" @input="$store.letterhead.save()"
                            placeholder="Company Name"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 outline-none focus:border-[#3b3c95]">
                        <input type="text" x-model="$store.letterhead.active.docTitle" @input="$store.letterhead.save()"
                            placeholder="Document Title (e.g. Statement)"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 outline-none focus:border-[#3b3c95]">
                        <input type="text" x-model="$store.letterhead.active.statementFor" @input="$store.letterhead.save()"
                            placeholder="Prepared For (optional)"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 outline-none focus:border-[#3b3c95]">
                        <textarea x-model="$store.letterhead.active.address" @input="$store.letterhead.save()"
                            placeholder="Address / Contact Details" rows="2"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 outline-none focus:border-[#3b3c95]"></textarea>
                        <input type="text" x-model="$store.letterhead.active.date" @input="$store.letterhead.save()"
                            placeholder="Document Date (blank = today)"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 outline-none focus:border-[#3b3c95]">
                        <div>
                            <label class="block text-[10px] text-[--ink-muted] uppercase mb-1">Date position</label>
                            <select x-model="$store.letterhead.active.datePosition" @change="$store.letterhead.save()"
                                class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 bg-white text-[--ink]">
                ...
                <div class="space-y-3 pt-4 border-t border-slate-100">
                    <div class="flex items-center justify-between">
                        ...
                        <input type="checkbox" :checked="$store.letterhead.showOnPrint"
                            @change="$store.letterhead.showOnPrint = $event.target.checked; $store.letterhead.save()"
                            class="rounded border-slate-300 text-[#3b3c95]">
                    </div>
                    <div class="space-y-2" x-show="$store.letterhead.showOnPrint">
                        ...
                        <select x-model="$store.letterhead.active.layout" @change="$store.letterhead.save()"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 bg-white text-[--ink]">
                        ...
                        <select x-model="$store.letterhead.active.size" @change="$store.letterhead.save()"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 bg-white text-[--ink]">
                    ...
                    <ul x-ref="columnsList" class="space-y-1.5">
                        <template x-for="col in $store.spreadsheet.headers" :key="col.key">
                            <li class="flex items-center gap-2 p-2 rounded-lg bg-slate-50 border border-slate-100 text-xs">
                                ...
                                <input type="checkbox" :checked="col.visible" @change="col.visible = !col.visible"
                                    class="rounded border-slate-300 text-[#3b3c95]">
                                <input type="text" x-model="col.label"
                                    class="bg-transparent border-none outline-none font-medium flex-1 min-w-0 text-[--ink]">
                ...
                <div class="space-y-2.5 pt-4 border-t border-slate-100">
                    ...
                    <label class="flex items-center justify-between text-xs text-[--ink-2] cursor-pointer">
                        ...
                        <input type="checkbox" :checked="$store.settings.showTotals"
                            @change="$store.settings.setShowTotals($event.target.checked)"
                            class="rounded border-slate-300 text-[#3b3c95]">
                    </label>
                    <label class="flex items-center justify-between text-xs text-[--ink-2] cursor-pointer">
                        <span>Bold last row</span>
                        <input type="checkbox" :checked="$store.settings.boldLastRow"
                            @change="$store.settings.setBoldLastRow($event.target.checked)"
                            class="rounded border-slate-300 text-[#3b3c95]">
                    </label>
                    <label class="flex items-center justify-between text-xs text-[--ink-2]">
                        <span>Table font size</span>
                        <span class="flex items-center gap-1">
                            <input type="number" min="7" max="24" step="1" :value="$store.settings.tableFontSize"
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <div class="flex items-center gap-1">
                                    <input type="color" :value="$store.settings.thBg || '#ffffff'"
                                        @input="$store.settings.setThBg($event.target.value)"
                                        class="w-6 h-6 rounded border border-[--border] cursor-pointer bg-transparent p-0">
                                    <button type="button" @click="$store.settings.setThBg('')"
                                        class="text-[9px] text-red-500 hover:text-red-600 font-medium"
                                        x-show="$store.settings.thBg">Clear</button>
                                </div>
                            </div>
                            <div>
                                <span class="text-[9px] text-[--ink-muted] block mb-1">Text Color</span>
                                <div class="flex items-center gap-1">
                                    <input type="color" :value="$store.settings.thText || '#000000'"
                                        @input="$store.settings.setThText($event.target.value)"
                                        class="w-6 h-6 rounded border border-[--border] cursor-pointer bg-transparent p-0">
                                    <button type="button" @click="$store.settings.setThText('')"
                                        class="text-[9px] text-red-500 hover:text-red-600 font-medium"
                                        x-show="$store.settings.thText">Clear</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-3 pt-4 border-t border-slate-100">
                    <div class="flex items-center justify-between">
                        ...
                        <input type="checkbox" :checked="$store.settings.showFooter"
                            @change="$store.settings.setShowFooter($event.target.checked)"
                            class="rounded border-slate-300 text-[#3b3c95]">
                    </div>
                    <div class="space-y-2" x-show="$store.settings.showFooter">
                        ...
                        <select :value="$store.settings.footerLayout"
                            @change="$store.settings.setFooterLayout($event.target.value)"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 bg-white text-[--ink]">
                        ...
                        <input type="text" :value="$store.settings.footerText"
                            @input="$store.settings.setFooterText($event.target.value)"
                            class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 bg-white text-[--ink]">
                    </div>
                </div>
                ...
                <div class="space-y-2 pt-4 border-t border-slate-100" x-show="$store.spreadsheet.dashboard.isGenerated">
                    ...
                    <p class="text-[11px] text-[--ink-muted] leading-snug">Opens a standalone report — KPIs, month grouping,
                        chart &amp; table — in its own window with its own controls. It never touches this document or the
                        printed table.</p>
                ...
                <div class="space-y-3 pt-4 border-t border-slate-100">
                    ...
                    <div class="space-y-2.5">
                        <div>
                            <label class="block text-[10px] text-[--ink-muted] uppercase mb-1">Date format <span
                                    class="normal-case text-slate-400">(date columns only)</span></label>
                            <select @change="$store.settings.setDateFormat($event.target.value)"
                                :value="$store.settings.dateFormat"
                                class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 bg-white text-[--ink]">
                        ...
                        <div>
                            ...
                            <select @change="$store.settings.setNumberFormat($event.target.value)"
                                :value="$store.settings.numberFormat"
                                class="w-full text-xs border border-[--border] rounded-lg px-3 py-2 bg-white text-[--ink]">
            ...
            <div class="flex-1 flex flex-col min-w-0">
                ...
                <div class="flex-1 flex flex-col justify-center items-center p-8 no-print bg-[#faf9f6] bg-grid" x-cloak
                    x-show="!$store.spreadsheet.isLoaded">
                    <div class="max-w-md w-full text-center space-y-6">
                        <div class="space-y-2">
                            <h2 class="text-xl font-bold text-[--ink]">Import spreadsheet to start</h2>
                            <p class="text-xs text-[--ink-muted]">Drop in your data files or browse local drives. Processing
                        ...
                        <div @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false"
                            ...
                            <input type="file" accept=".xlsx,.xls,.csv,.tsv" class="hidden" x-ref="toolInput"
                                @change="handleFile($event)">

                            <div class="space-y-3">
                                <div
                                    class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center mx-auto group-hover:scale-105 transition-transform duration-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.8" stroke="currentColor" class="w-5 h-5 text-[#3b3c95]">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="document-preview-pane flex-1 w-full" x-cloak x-show="$store.spreadsheet.isLoaded"
                    ...
                    <div class="no-print max-w-[210mm] mx-auto mt-6 -mb-2 px-1">
                        <div
                            class="flex items-start gap-2 rounded-lg border border-[#3b3c95]/15 bg-[#3b3c95]/5 px-4 py-2.5 text-[11px] leading-relaxed text-[#454570]">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8"
                                stroke="currentColor" class="w-4 h-4 shrink-0 text-[#3b3c95] mt-px">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z" />
                            </svg>
                            <span>This is a working preview — a wide table may overflow the sheet here. <span
                                    class="font-semibold text-[#141428]">Print / Save PDF opens the Print Preview, which
                                    fits the whole table within the page margins.</span> Try Portrait vs. Landscape there
                    ...
                    <div id="print-area" class="a4-page-container transition-all">
                        ...
                        <div x-show="$store.letterhead.showOnPrint" class="mb-6">
                            <div class="border-b-2 border-[#3b3c95] pb-6 mb-8 flex justify-between items-start"
                                x-show="$store.letterhead.active.layout === 'split-header'">
                                <div class="space-y-1">
                                    <h2 class="text-xl font-bold tracking-tight text-slate-900"
                                        x-text="$store.letterhead.active.companyName || 'Company Name'"></h2>
                                    <p class="text-xs text-slate-500 whitespace-pre-line leading-relaxed"
                                        x-text="$store.letterhead.active.address || 'Address information'"></p>
                                </div>
                                <div class="text-right space-y-1 font-mono">
                                    <h3 class="text-sm font-bold text-[#3b3c95] tracking-wider uppercase"
                                        x-text="$store.letterhead.active.docTitle || 'DOCUMENT'"></h3>
                                    <p class="text-xs text-slate-500"
                                        x-show="$store.letterhead.active.datePosition === 'top'"
                                        x-text="'Date: ' + docDate()"></p>
                                    <p class="text-xs text-slate-500" x-show="$store.letterhead.active.statementFor"
                                        x-text="'For: ' + $store.letterhead.active.statementFor"></p>
                                </div>
                            </div>

                            <!-- Minimal rule -->
                            <div class="text-center pb-4 mb-6 border-b border-slate-900"
                                x-show="$store.letterhead.active.layout === 'minimal-rule'">
                                <h2 class="text-lg font-bold" x-text="$store.letterhead.active.companyName"></h2>
                                <p class="text-[10px] text-slate-500"
                                    x-text="$store.letterhead.active.docTitle + ($store.letterhead.active.datePosition === 'top' ? ' · ' + docDate() : '')">
                                </p>
                            </div>

                            <!-- Centered -->
                            <div class="text-center pb-6 mb-8 border-b-2 border-[#3b3c95]"
                                x-show="$store.letterhead.active.layout === 'centered'">
                                <h2 class="text-xl font-bold tracking-tight text-slate-900"
                                    x-text="$store.letterhead.active.companyName || 'Company Name'"></h2>
                                <p class="text-xs text-slate-500 mt-0.5 whitespace-pre-line"
                                    x-text="$store.letterhead.active.address"></p>
                                <div
                                    class="mt-2 flex items-center justify-center gap-3 text-[10px] font-mono text-slate-500">
                                    <span class="font-bold uppercase tracking-wider text-[#3b3c95]"
                                        x-text="$store.letterhead.active.docTitle || 'DOCUMENT'"></span>
                                    <span x-show="$store.letterhead.active.datePosition === 'top'"
                                        x-text="'Date: ' + docDate()"></span>
                                    <span x-show="$store.letterhead.active.statementFor"
                                        x-text="'For: ' + $store.letterhead.active.statementFor"></span>
                                </div>
                            </div>

                            <!-- Left accent bar -->
                            <div class="flex gap-3 pb-6 mb-8 border-b border-slate-200"
                                x-show="$store.letterhead.active.layout === 'left-accent-bar'">
                            ...
                            <div class="flex items-center justify-between pb-6 mb-8 border-b-2 border-[#3b3c95]"
                                x-show="$store.letterhead.active.layout === 'monogram-inline'">
                                <div class="flex items-center gap-3">
                                    <div class="w-11 h-11 rounded-full bg-[#3b3c95]/10 text-[#3b3c95] flex items-center justify-center font-black text-lg shrink-0"
                                        x-text="($store.letterhead.active.companyName || 'My').trim().slice(0,2).toUpperCase()">
                                    </div>
                                    <div class="space-y-0.5">
                                        <h2 class="text-lg font-bold tracking-tight text-slate-900"
                                            x-text="$store.letterhead.active.companyName || 'Company Name'"></h2>
                            ...
                            <div class="border-y-2 border-[#3b3c95] py-4 mb-8 flex justify-between items-start"
                                x-show="$store.letterhead.active.layout === 'split-bordered'">
                                <div class="space-y-1 flex items-center gap-4">
                                    <img x-show="$store.letterhead.active.logoBase64"
                                        :src="$store.letterhead.active.logoBase64" class="shrink-0"
                                        :style="'height: ' + $store.letterhead.active.logoHeight + 'px'">
                                    <div>
                                        <h2 class="text-xl font-bold tracking-tight text-slate-900"
                                            x-text="$store.letterhead.active.companyName || 'Company Name'"></h2>
                                        <p class="text-xs text-slate-500 whitespace-pre-line leading-relaxed"
                            ...
                            <div class="grid grid-cols-3 gap-6 pb-6 mb-8 border-b-2 border-slate-200"
                                x-show="$store.letterhead.active.layout === 'editorial-column'">
                                <div class="col-span-2 border-r border-slate-200 pr-6 flex items-start gap-4">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-[11px] text-slate-400 italic"
                                            x-show="$store.letterhead.active.tagline"
                                            x-text="$store.letterhead.active.tagline"></p>
                                ...
                                <div class="pl-2 space-y-2 text-right self-end font-mono shrink-0">
                                    ...
                                    <div class="text-[10px] text-slate-500 space-y-0.5">
                                        <p x-show="$store.letterhead.active.datePosition === 'top'"
                                            x-text="'Date: ' + docDate()"></p>
                                        <p x-show="$store.letterhead.active.statementFor"
                                            x-text="'For: ' + $store.letterhead.active.statementFor"></p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start justify-between pb-6 mb-8 border-b border-slate-100"
                                x-show="$store.letterhead.active.layout === 'modern-minimalist'">
                                ...
                                <div class="text-right text-xs text-slate-500 font-mono leading-tight shrink-0">
                                    <h3 class="font-bold text-[#3b3c95] uppercase text-xs"
                                        x-text="$store.letterhead.active.docTitle || 'DOCUMENT'"></h3>
                                    <p class="mt-1" x-show="$store.letterhead.active.datePosition === 'top'"
                                        x-text="'Date: ' + docDate()"></p>
                                    <p x-show="$store.letterhead.active.statementFor"
                                        x-text="'For: ' + $store.letterhead.active.statementFor"></p>
                                </div>
                            ...
                            <div class="bg-slate-50 rounded-xl p-5 mb-8 border border-slate-200/80"
                                x-show="$store.letterhead.active.layout === 'corporate-block'">
                            ...
                            <div class="flex items-start justify-between pb-6 mb-8 border-b-2 border-dashed border-slate-200"
                                x-show="$store.letterhead.active.layout === 'asymmetric-compact'">
                                <div class="space-y-1">
                                    <div class="flex items-center gap-3">
                                        <div x-show="!$store.letterhead.active.logoBase64"
                                    ...
                                    <p class="text-xs text-slate-500 max-w-md pt-1"
                                        x-text="$store.letterhead.active.address"></p>
                                </div>
                            ...
                            <div class="grid grid-cols-3 gap-6 pb-6 mb-8 border-b-2 border-slate-200 text-xs text-slate-700"
                                x-show="$store.letterhead.active.layout === 'compact-grid'">
                                <div class="space-y-1">
                                    ...
                                    <p class="text-[10px] text-slate-500 whitespace-pre-line leading-tight"
                                        x-text="$store.letterhead.active.address"></p>
                                </div>
                                <div class="border-l border-slate-200 pl-6 space-y-1 font-mono">
                                    <h3 class="font-bold uppercase text-[#3b3c95]"
                                        x-text="$store.letterhead.active.docTitle || 'DOCUMENT'"></h3>
                                    <p class="text-[10px] text-slate-500"
                                        x-show="$store.letterhead.active.datePosition === 'top'"
                                        x-text="'Date: ' + docDate()"></p>
                                ...
                                <div class="border-l border-slate-200 pl-6 space-y-1">
                                    <p class="font-semibold text-slate-700" x-show="$store.letterhead.active.statementFor"
                                        x-text="'For: ' + $store.letterhead.active.statementFor"></p>
                                    <p class="text-[10px] text-slate-500" x-show="$store.letterhead.active.bin"
                                        x-text="'BIN: ' + $store.letterhead.active.bin"></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <table class="doc-table"
                            :style="{ 'font-size': computedTableFontSize + 'px', '--th-bg': $store.settings.thBg || undefined, '--th-text': $store.settings.thText || undefined }">
                            <thead>
                                <tr>
                                    <template x-for="col in $store.spreadsheet.headers.filter(h => h.visible)"
                                        :key="col.key">
                                        <th :class="col.align === 'right' ? 'align-right' : ''">
                                            <span class="inline-flex items-center gap-1 max-w-full align-middle"
                                                ...
                                                <input type="text" x-model="col.label" @keydown.enter="$event.target.blur()"
                            ...
                            <tbody>
                                <template x-for="(row, idx) in sortedRows" :key="idx">
                                    <tr
                                        :class="$store.settings.boldLastRow && idx === sortedRows.length - 1 ? 'pt-bold-row' : ''">
                                        <template x-for="col in $store.spreadsheet.headers.filter(h => h.visible)"
                                            :key="col.key">
                            ...
                            <tfoot x-show="$store.settings.showTotals && hasNumericColumns()">
                                <tr>
                                    <template x-for="(col, i) in $store.spreadsheet.headers.filter(h => h.visible)"
                                        :key="col.key">
                                        <td :class="col.align === 'right' ? 'align-right' : ''"
                        ...
                        <div x-show="$store.letterhead.showOnPrint && $store.letterhead.active.datePosition === 'bottom'"
                            class="mt-8 text-right text-xs font-mono text-slate-500" x-text="'Date: ' + docDate()"></div>

                        <!-- Dynamic Footnote Footer (Max 1 Line) -->
                        <div x-show="$store.settings.showFooter"
                            class="mt-6 border-t border-slate-100 pt-2 text-[10px] text-slate-400 font-mono">
                            <!-- Simple Text Layout -->
                            <div class="text-center" x-show="$store.settings.footerLayout === 'simple-text'"
                                x-text="$store.settings.footerText"></div>
                            ...
                            <div class="space-y-1" x-show="$store.settings.footerLayout === 'brand-accent'">
                                ...
                                <div class="text-center"
                                    x-text="($store.letterhead.active.companyName || 'Paper Trail') + ' · ' + $store.settings.footerText">
                                </div>
                            </div>

                            <!-- Split Footnote Layout -->
                            <div class="flex justify-between items-center"
                                x-show="$store.settings.footerLayout === 'split-footnote'">
                                <span class="font-bold"
                                    x-text="$store.letterhead.active.companyName || 'Paper Trail'"></span>
